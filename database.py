...
import os as _os
DB_PATH = _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), "quiz_bot.db")
...
def _migrate_db():
    """Purane DB mein naye columns add karo — safe migration."""
    c = _conn()
    # question_sets mein subject_id aur topic_id add karo agar missing hain
    try:
        c.execute("SELECT subject_id FROM question_sets LIMIT 1")
    except Exception:
        try:
            c.execute("ALTER TABLE question_sets ADD COLUMN subject_id INTEGER REFERENCES subjects(id) ON DELETE SET NULL")
            c.commit()
        except Exception:
            pass
    try:
        c.execute("SELECT topic_id FROM question_sets LIMIT 1")
    except Exception:
        try:
            c.execute("ALTER TABLE question_sets ADD COLUMN topic_id INTEGER REFERENCES topics(id) ON DELETE SET NULL")
            c.commit()
        except Exception:
            pass
    # leaderboard mein section_tag add karo agar missing hai
    try:
        c.execute("SELECT section_tag FROM leaderboard LIMIT 1")
    except Exception:
        try:
            c.execute("ALTER TABLE leaderboard ADD COLUMN section_tag TEXT DEFAULT 'overall'")
            c.commit()
        except Exception:
            pass
    # leaderboard UNIQUE constraint update — agar purana hai toh recreate
    try:
        # Check if unique constraint includes section_tag
        info = c.execute("SELECT sql FROM sqlite_master WHERE type='table' AND name='leaderboard'").fetchone()
        if info and 'section_tag' not in info[0]:
            ...
            c.executescript("""
                ...
                INSERT OR IGNORE INTO leaderboard_new(chat_id,user_id,name,score,correct,wrong,quizzes,ts)
                SELECT chat_id,user_id,name,score,correct,wrong,quizzes,ts FROM leaderboard;
        ...
        import logging
        logging.getLogger(__name__).warning(f"leaderboard migration: {e}")
