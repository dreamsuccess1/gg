...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if is_admin(user.id):
        text += (
            ...
            "/resetscores — Reset scores\n"            "\n"            "✅ *Auto-Save:*\n"            "/addquestion — ✅ wale questions auto-save karein\n"            "/done — Auto-save mode बंद करें"
        )
...
async def addquestion_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Admin /addquestion bheje → bot bolta hai ✅ wala question bhejo."""
    if not is_admin(update.effective_user.id):
        return
    ctx.user_data.clear()
    ...
    await update.message.reply_text(
        "📌 *Auto-Save Mode*\n\n"
        "Ab koi bhi question bhejein jisme sahi option ke aage *✅* laga ho।\n\n"
        "Example:\n"
        "`भारत की राजधानी?\n"
        "(A) मुंबई\n"
        "(B) दिल्ली ✅\n"
        "(C) कोलकाता\n"
        "(D) चेन्नई`\n\n"
        "Bot khud detect karke set chunne ka option dega।\n"
        "/done — khatam karein",
        parse_mode=ParseMode.MARKDOWN
    )

async def addquestion_done(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    if ctx.user_data.get("aq_mode") or ctx.user_data.get("aq_waiting_setname") or ctx.user_data.get("aq_waiting_fwdsetname"):
        ctx.user_data.clear()
        await update.message.reply_text("✅ Auto-Save mode बंद।")
    else:
        await update.message.reply_text("कोई active mode नहीं।")
...
async def handle_aq_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if ctx.user_data.get("aq_waiting_fwdsetname"):
        ...
        return
    # Only parse ✅ question if in aq_mode OR ✅ detected (always-on for admin)
    # ✅️ (variation selector ke saath) aur ✅ dono handle karo
    text = update.message.text or update.message.caption or ""
    text = text.replace("✅️", "✅")   # normalize variation selector U+FE0F
    if "✅" not in text:
        return
    parsed = parse_checkmark_question(text)
    if not parsed:
        if ctx.user_data.get("aq_mode"):
            await update.message.reply_text(
                "⚠️ Format detect नहीं हुआ।\n"
                "सुनिश्चित करें कि question और options अलग-अलग lines में हों और सही option पर ✅ लगा हो।"
            )
        return
...
async def aqset_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Set selection for /addquestion flow."""
    query = update.callback_query
    await query.answer()
    if not is_admin(query.from_user.id):
        return
...
async def _do_save_aq(msg, ctx, set_id: int):
    ...
    set_info = db.get_set(set_id)
    labels   = ["A","B","C","D","E"]
    # Clear only this question's data, keep aq_mode
    for k in ["aq_q","aq_opts","aq_correct","aq_photo"]:
        ctx.user_data.pop(k, None)
    text = (
        f"✅ *सवाल save हो गया!*\n\n"
        f"❓ {q}\n"
        f"✔️ सही जवाब: *{labels[correct] if correct<len(labels) else correct+1}: {opts[correct]}*\n"
        f"📂 Set: *{set_info['name']}*\n\n"
...
async def fwdset_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "fwdset_cancel":
        ctx.user_data.clear()
        await query.message.edit_text("❌ Poll skip किया।")
        return
    if data == "fwdset_new":
...
def build_app():
    ...
    # ✅ /addquestion — Auto ✅ detect set selector
